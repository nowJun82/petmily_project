...
public class SiteUser implements OAuth2User, UserDetails {
    ...
    @Column(unique = true)
    private String username; // 사용자 이름은 고유해야 함
    private String password; // 사용자의 암호화된 비밀번호 발급
    ...
    // UserDetails 인터페이스 구현 -> 현재 OAuth2.0 인증을 사용하고 있기에 Spring Security 를 사용하지 않는다면
    // UserDetails 구현은 필요가 없다. 근데, 혹시 모르니 남겨둔다.
    // 일단 오류는 없기 때문이다.
    @Override
